...
const BlossomMap = (() => {
  ...
  function init() {
    ...
    clusterGroup = L.markerClusterGroup({
      ...
      iconCreateFunction: (cluster) => {
        ...
        return L.divIcon({
          html: `<div class="blossom-cluster ${sizeClass}">${count}</div>`,
          className: '',
  ...
  function addBlossom(drop, animate = false) {
    const icon = L.divIcon({
      html: `<div class="cherry-marker ${animate ? 'cherry-marker-new' : ''}">${treeSVG(44)}</div>`,
      className: '',
      iconSize: [44, 52],
      iconAnchor: [22, 52]
    });
    ...
    // Add glow effect after boom animation
    if (animate) {
      setTimeout(() => {
  ...
  return { init, addBlossom, getMap, getClusterGroup };
})();
